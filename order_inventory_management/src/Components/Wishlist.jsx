...
export default function Wishlist({isLoggedIn, user}) {
    ...
    const fetchWishlist = async() => {
        ...
        // try {
        //     const response = await fetch(`http://localhost:3000/users/getWishlist?userId=${user._id}`,{
        //         method: 'GET',
        //         headers: {
            ...
            <div className="space-y-6 mt-6">
                {products.length > 0 ? (
                    products.map((product) => (
                        <div key={product.productId._id} className="flex items-start justify-between p-6 bg-white rounded-lg shadow-md border border-gray-200 hover:shadow-lg transition-shadow">
                        ...
                        {product.productId.images && product.productId.images.length > 0 ? (
                            ...
                            <span className="text-gray-500 text-sm">No Image</span>
                                ...
                                <div className="space-y-2 flex-1">
                                    <h4 className="text-lg font-medium">{product.productId.name}</h4>
                                    <p className="text-gray-500">₹{product.productId.price}</p>
                                    
                                    {product.productId.discountPercentage > 0 && (
                                        <p className="text-sm text-green-600">Discount: {product.productId.discountPercentage}% off</p>
                                    )}
                                    ...
                                    <p className="text-sm text-gray-700">{product.productId.description}</p>
                                </div>
                            </div>
